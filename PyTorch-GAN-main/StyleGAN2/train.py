...
def train(args, loader, generator, discriminator, g_optim, d_optim, g_ema, device, fid_record, sample_z):
    ...
    for idx in pbar:
        ...
        if args.regan:
            if generator.train_on_sparse:
                generator.apply_masks()
                for mask_w, ema_w in zip(generator.masks, g_ema.layers):
                    ema_w.data[mask_w] = 0
        ...
        if i % args.eva_iter == 0:
            with torch.no_grad():
                ...
                for ii in tqdm(range(args.eva_size // eva_bs), desc='Generating Images'):
                    sample_z_fid = torch.randn(eva_bs, args.latent, device=device)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--diffaug", action="store_true", help="apply DiffAug"
    )
    parser.add_argument(
        "--regan", action="store_true", help="ReGAN training mode or not"
    )
    parser.add_argument('--sparsity', type=float, default=0.1)
    parser.add_argument('--g', type=int, default=10)
    parser.add_argument('--eva_size', type=int, default=5000)
    parser.add_argument('--eva_iter', type=int, default=1000)
    parser.add_argument('--warmup_iter', type=int, default=100)
    ...
    if args.regan:
        ...
        root_dir = './results/%s' % args.dataset
    ckpt_dir = root_dir + '/checkpoint'
    eva_dir = root_dir + '/eva'
    ...
    if args.regan:
        generator = Regan_training(generator, args.sparsity)
        g_ema = Regan_training(g_ema, args.sparsity)
    ...
    if args.ckpt is not None:
        ...
        fid_record = ckpt['fid_record']
        args.start_iter = ckpt['saved_iter']
        sample_z = ckpt['sample_z'].to(device)
    else:
        fid_record = [99999]
        sample_z = torch.randn(args.n_sample, args.latent, device=device)

    best_fid = min(fid_record)
